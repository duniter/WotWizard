...
type (
	...
	transLex []Anyptr	// *gotoLexC or *gotoLexT
...
type (
	...
	Compilationer interface {
		Read () (ch rune, cLen int)	// Text reading method: ch is the next character read. When reading after the end of text, ch must return EOF1 or EOF2. 
		Pos () int	// Returns the current position in the input stream of chars (origin can have any value). 
		SetPos (pos int)	// Moves the current position in the input stream of chars to pos (origin must be the same than for Pos). pos may extend until the position which follows the end of the text. 
		Execution (fNum, parsNb int, pars ObjectsList) (objPos *Object, res Anyptr, ok bool)	// Execution of hard functions: fNum is the index of the hard function, parsNb is the number of input parameters; pars is the list of input parameters; objPos is an Object whose position in text will be the position of res; res is the result of the hard
		Error (pos, line, col int, msg string)	// Lexical and syntactic errors notification: pos position, line line number and col column number of the error; msg is the error message. pos, line and col are numbered from 1. 
		Map (index string) string	// Maps an index text to a more lengthy one. Used to make explicit or to translate error messages. 
	}
...
func (c *Compilation) erreurLex (n, li, co, p int) {
	...
	switch n {
		case 1: err("Unexpected character.")
		case 2: err("Opened comment at the end of text.")
		case 3: err("Unexpected end of comment.")
	}
}
