...
func (c *compilation) Read () (ch rune, cLen int) {
	ch, cLen, err := c.r.ReadRune()
	M.Assert(err == nil || err == io.EOF, err, 100)
	if err != nil {
		ch = C.EOF2
		cLen = 1
		return
...
func (c *compilation) SetPos (pos int) {
	if pos <= c.size {
		_, err := c.rs.Seek(int64(pos - 1), io.SeekStart); M.Assert(err == nil, err, 100)
		c.r.Reset(c.rs)
		c.pos = pos
	}
}
...
func Compile (rs io.ReadSeeker) (SelectionSet, *A.Tree) {
	...
	co := &compilation{r: bufio.NewReader(rs), rs: rs, size: int(n), pos: 1, err: A.New(), sel: nil}
