...
func (c1 *certif) Compare (c2 A.Comparer) A.Comp {
	cc2 := c2.(*certif)
	if c1.exp < cc2.exp {
		return A.Lt
	}
...
func DoCertifsEnds () Memberships {
	...
	for ok {
		...
		if bb {
			cT := A.New()
			from, to, ok2 := pos.CertNextPos()
			for ok2 {
				c := new(certif)
				var b bool
				c.from, b = B.IdPub(from); M.Assert(b, 102)
				_, c.exp, b = B.Cert(from, to); M.Assert(b, 103)
				_, b, _ = cT.SearchIns(c); M.Assert(!b, 104)
				from, to, ok2 = pos.CertNextPos()
			}
			e, b := cT.Find(cT.NumberOfElems() - int(B.Pars().SigQty) + 1); M.Assert(b, 105)
			ms.m[i] = membershipC{membership: membership{id: id, exp: e.Val().(*certif).exp}, member: mem}
			i++
		}
		id, ok = B.IdNextUid(false, &ir)
	}
