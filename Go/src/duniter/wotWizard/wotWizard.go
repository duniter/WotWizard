...
func fileDates (f File, afterNow bool) {
	if f != nil {
		var now int64
		if afterNow {
			now = B.Now()
		...
		for i := 0; i < len(f); i++ {
			...
			case *Dossier:
				if cd.Certifs == nil {
					...
					if afterNow {
						cd.date = M.Max64(cd.Certifs[cd.PrincCertif - 1].(*Certif).date, now)
					} else {
...
func sortFile (f File, i0 int, afterNow bool) {
	if f != nil {
		for i := i0; i < len(f); i++ {
			switch cd := f[i].(type) {
			case *Dossier:
...
func FillFile (minCertifs int) (f File, cNb, dNb int) {
	...
	if dNb == 0 {
		...
		sortFile(f, 0, minCertifs >= int(B.Pars().SigQty))
	}
	return
}
