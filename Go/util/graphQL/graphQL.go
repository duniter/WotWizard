...
Types:
	...
	SourceEventNotification func

Implementations:
		
	EventStreamer	implemented in U; interface{RecordNotificationProc, CloseEvent}; Put in EventStream by MakeEventStream
	
	ResponseEventManager	implemented in U; interface{ManageResponseEvent}
	
	every StreamResolver(s)

Calls to graphQL:
	
	FixStreamResolver (fieldName, StreamResolver)	U -> G; Fixes the StreamResolver for fieldName
	
	MakeEventStream (es EventStreamer) EventStream	U -> G; Makes an EventStream from an EventStreamer
	
	RecordResponseEventManager (ResponseStream, ResponseEventManager)	U -> G; records the ResponseEventManager for ResponseStream; called during the treatment of a SubscribeResponse
	
	SourceEventNotification (EventStream, event)	U -> G; Sends an event for EventStream
	
	Unsubscribe (ResponseStream)	U -> G; calls ResponseStream.EventStream.EventStreamer.CloseEvent

Calls from graphQL:
	
	CloseEvent ()	G -> U; In EventStreamer; Used by Unsubscribe
	
	ManageResponseEvent (ResponseStream, Response)	G -> U; In ResponseEventManager; manages Response for ResponseStream
	
	RecordNotificationProc (SourceEventNotification)	G -> U; In EventStreamer; Records a SourceEventNotification
	
	StreamResolver (rootValue, argumentValues) EventStream	G -> U; Creates and returns an EventStream for rootValue and argumentValues
*/
...
type (
	...
	ResponseEventManager interface {
		ManageResponseEvent (rs *ResponseStream, r Response)
	}
...
func (ts *error) SetErrors (errors *A.Tree) {
	ts.errors = errors
}

func (ts *error) GetErrors () *A.Tree { // *ErrorElem
	return ts.errors
}
...
func (ts *error) Error (mes, aux1, aux2 string, pos *PosT, path Path) {
...
func Compile (rs io.ReadSeeker) (*Document, Response) {// *ErrorElem
	...
	r := []rune(string(b))
	co := &compilationer{r: r, size: len(r), pos: 0, err: A.New(), doc: nil}
...
type (
	...
	// LinkGQL(name) returns the ReadSeekCloser corresponding to name
	LinkGQL func (name string) ReadSeekCloser
...
func (rs *ResponseStream) RecordResponseEventManager (manager ResponseEventManager) {
	rs.responseEventManager = manager
} //RecordResponseEventManager
...
func getSourceEvent (es *EventStream, event *OutputObjectValue) {
	M.Assert(es.ResponseStream.responseEventManager != nil, 100)
	es.ResponseStream.responseEventManager.ManageResponseEvent (es.ResponseStream, es.es.executeSubscriptionEvent(es.subscription, es.variableValues, event))
} //getSourceEvent
...
func (es *execSystem) mapSourceToResponseEvent (sourceStream *EventStream, subscription *OperationDefinition, variableValues *A.Tree) *ResponseStream { // *ValMapItem
	sourceStream.es = es
	sourceStream.subscription = subscription
	sourceStream.variableValues = variableValues
	responseStream := &ResponseStream{SourceStream: sourceStream}
	sourceStream.ResponseStream = responseStream
...
func linkFile (name string) ReadSeekCloser {
