...
func compile1Text (input io.ReadSeeker, output, binOutput io.Writer) (path, name string, ok bool) {
...
func main () {
	l := os.Args[1:]
	if len(l) == 0 {
		fmt.Fprintln(os.Stderr, "$babel {path_from_rsrc_to_babel_file_without_'bab'}")
	} else {
		compileList(l)
	}
}
